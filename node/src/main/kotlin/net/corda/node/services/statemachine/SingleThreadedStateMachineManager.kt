...
class SingleThreadedStateMachineManager(
    ...
    private fun onSessionInit(sessionMessage: InitialSessionMessage, sender: Party, event: ExternalEvent.ExternalMessageEvent) {
        try {
            ...
            logger.warn("Unable to initiate flow for $sender (appName=${sessionMessage.appName} " +
                    "flowVersion=${sessionMessage.flowVersion}), sending to flow hospital", t)
            flowHospital.sessionInitError(sessionMessage, sender, event, t)
        }
    }
