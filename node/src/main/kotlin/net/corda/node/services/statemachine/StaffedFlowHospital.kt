...
class StaffedFlowHospital(private val flowMessaging: FlowMessaging, private val ourSenderUUID: String) {
    ...
    private val mutex = ThreadBox(object {
        ...
        val treatableSessionInits = HashMap<Long, InternalSessionInitRecord>()
        val recordsPublisher = PublishSubject.create<MedicalRecord>()
    })
    private val secureRandom = newSecureRandom()

    fun sessionInitError(sessionMessage: InitialSessionMessage, sender: Party, event: ExternalEvent.ExternalMessageEvent, error: Throwable) {
        val time = Instant.now()
        val id = secureRandom.nextLong()
        val outcome = if (error is SessionRejectException.UnknownClass) {
            // We probably don't have the CorDapp installed so let's leave the message un-acked so that it can be re-delivered
            // on restart
    ...
    fun dropSessionInit(id: Long) {
        val (sessionMessage, event, publicRecord) = mutex.locked {
            requireNotNull(treatableSessionInits.remove(id)) { "Unknown id $id for errored session init" }
        }
    ...
    sealed class MedicalRecord {
        ...
        data class SessionInit(val id: Long,
